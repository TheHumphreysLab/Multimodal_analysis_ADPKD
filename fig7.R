...
fig7a_1 <- DimPlot(sub_atac,raster=FALSE) #450x330
...
fig7a_2 <- DimPlot(CNT_PCatac)+NoLegend() #550x440
Idents(CNT_PCatac) <- "disease"
levels(CNT_PCatac) <- c("PKD","Cont")
fig7a_3 <- DimPlot(CNT_PCatac)+NoLegend()
...
fig7b <- DotPlot(CNT_PCatac, features = features, cols = c("lightyellow","royalblue")) +
  RotatedAxis() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) #600x450
levels(CNT_PCatac) <- rev(levels(CNT_PCatac))

DefaultAssay(CNT_PCatac) <- "chromvar"
fig7c_1 <- FeaturePlot(CNT_PCatac,"MA0105.3",cols =jdb_palette("brewer_yes")) #550x500
fig7c_2 <- FeaturePlot(CNT_PCatac,"MA0107.1",cols =jdb_palette("brewer_yes")) #550x500
fig7c_3 <- FeaturePlot(CNT_PCatac,"MA0808.1",cols =jdb_palette("brewer_yes")) #550x500 tead3

fig7f_1 <- FeaturePlot(CNT_PCatac,"MA0018.3",cols =jdb_palette("brewer_yes")) #550x500 CREB1
fig7f_2 <- FeaturePlot(CNT_PCatac,"MA1149.1",cols =jdb_palette("brewer_yes")) #550x500 RARA::RXRG
levels(CNT_PCatac) <- c("PKD","Cont")
fig7f_3 <- VlnPlot(CNT_PCatac,"MA0018.3",pt.size = 0) #550x500 CREB1
fig7f_4 <- VlnPlot(CNT_PCatac,"MA1149.1",pt.size = 0) #550x500 RARA::RXRG
...
contigs <- read.table("/Users/mutouyoshiharu/Desktop/diabneph/github_repository/healthyKidney/utility_scripts/contigLengths.txt")
...
saveRDS(conns,"/Users/mutouyoshiharu/Desktop/PKD_seq/Fig_v4/fig7/pkd_conns_chr12.rds")

gene_annotation <- read.table("/Users/mutouyoshiharu/Desktop/PKD_seq/ATAC/human_hg38_annotation.txt")
gene_annotation$feature <- as.character(gene_annotation$feature)

fig7e_1 <- plot_connections(conns, "chr12", 12870000, 12895000,
...
conns <- run_cicero(cicero_cds, contigs)
saveRDS(conns,"/Users/mutouyoshiharu/Desktop/PKD_seq/Fig_v4/fig7/pkd_conns_chr9.rds")

figS7d_1 <- plot_connections(conns, "chr9", 21400000,22050000,
                 gene_model = gene_annotation,
                 coaccess_cutoff = .2, 
                 connection_width = .5, 
                 collapseTranscripts = "longest" ) #600x300
